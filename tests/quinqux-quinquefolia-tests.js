...
test('No.106の基本データと指定アイコンを登録', () => {
  ...
  assert.deepStrictEqual(servant.skillIcons, ['skill-unique-032.png', 'skill-np-charge.png', 'skill-general-062.png']);
  assert.deepStrictEqual(servant.passives.map((passive) => passive.icon), [
    'class-general-009.png', 'skill-buster-up.png', 'skill-general-033.png', 'skill-general-024.png', 'skill-non.png'
  ]);
...
test('誰のものでもない仮面は先頭の他味方へスキルと宝具を換装し指定アイコンを表示', () => {
  const e = engine([
  ...
  assert.strictEqual(firstOther.statuses.find((s) => s.type === MECHANICS.statusTypes.transformed).statusIcon, 'Dragontrait.webp');
  assert.strictEqual(firstOther.statuses.find((s) => s.type === MECHANICS.statusTypes.skillOneSeal).statusIcon, 'Skillseal.webp');
...
test('クラススキル状態は指定アイコンで付与される', () => {
  const e = engine([
    { servantId: 'quinquxQuinquefolia' },
    { servantId: 'fenrir' },
    { servantId: 'koyanskayaLight' },
    { servantId: 'skadiCaster' }
  ]);
  const [quinqux, ally, other, reserve] = e.getState().allies;
  assert.strictEqual(quinqux.statuses.find((s) => s.type === 'mentalDebuffImmune').statusIcon, 'Debuffimmune.webp');
  assert.strictEqual(quinqux.statuses.find((s) => s.type === 'skillSealImmune').statusIcon, 'Debuffimmune.webp');
  assert.strictEqual(quinqux.statuses.find((s) => s.type === MECHANICS.statusTypes.winnerGlory).statusIcon, 'Critdmgup.webp');
  [ally, other, reserve].forEach((unit) => {
    const applause = unit.statuses.find((s) => s.type === MECHANICS.statusTypes.audienceApplause);
    assert.ok(applause);
    assert.strictEqual(applause.statusIcon, 'Attackup.webp');
    assert.strictEqual(applause.value, 20);
  });
  assert.strictEqual(quinqux.statuses.some((s) => s.type === MECHANICS.statusTypes.audienceApplause), false);
});
