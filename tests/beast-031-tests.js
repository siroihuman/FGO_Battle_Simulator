...
function engine(party, enemies = [enemy('対象')]) {
...
test('怨讐の畔は控えを含む味方全体の弱体耐性を15低下', () => {
  ...
  e.getState().allies.forEach((ally) => {
    assert.strictEqual(e._statusTotal(ally, 'debuffResist'), -15);
  });
});

test('永劫の罪過は叛逆する者からNP25を吸収しターン終了時に還元', () => {
  const e = engine([
...
test('鏖殺の獣は敵強化解除・即死耐性低下・毎ターンNP等を付与', () => {
...
test('宝具前効果は憎悪と憎悪特攻をダメージ前に付与', () => {
  const e = engine([{ servantId: 'beast031', startingNp: 100 }]);
  const beast = e.getState().allies[0];
  const target = e.getState().enemies[0];
  const np = beast.data.np;
  np.before.forEach((effect) => e._applyEffect(effect, beast, beast.id, { oc: 1, level: 10 }));
