...
const DURATION_TEST_SOURCE = '控え状態ターン検証';
...
function durationTestTurns(unit) {
  return (unit.statuses || [])
    .filter((status) => status.source === DURATION_TEST_SOURCE)
...
test('控えサーヴァントの強化・弱体の残りターンは減少しない', () => {
  ...
  frontline.statuses.push(
    { type: 'attackUp', value: 10, remaining: 3, uses: null, debuff: false, source: DURATION_TEST_SOURCE },
    { type: 'defenseDown', value: 10, remaining: 3, uses: null, debuff: true, source: DURATION_TEST_SOURCE }
  );
  ...
  assert.deepStrictEqual(durationTestTurns(frontline), [2, 2]);
  assert.deepStrictEqual(durationTestTurns(reserve), [3, 3]);
});
...
test('控えから前衛へ出た後は状態の残りターンが減少する', () => {
  ...
  const status = {
    ...
    uses: null,
    debuff: false,
    source: DURATION_TEST_SOURCE
  };
  reserve.statuses.push(status);
...
test('index.htmlが敵行動設定後・タイムライン前にランタイムを読み込む', () => {
  const html = fs.readFileSync(path.join(__dirname, '../index.html'), 'utf8');
  assert.ok(html.includes('js/enemy-turn-charge-reserve-status.js'));
  assert.ok(html.indexOf('js/enemy-turn-charge-reserve-status.js') > html.indexOf('js/enemy-action-defaults.js'));
  assert.ok(html.indexOf('js/enemy-turn-charge-reserve-status.js') < html.indexOf('js/turn-action-timeline.js'));
});
